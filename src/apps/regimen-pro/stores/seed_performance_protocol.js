...
async function seedPerformanceProtocol() {
    ...
    try {
        ...
        // CLEANUP WITH CASCADE-MIMIC (Delete logs first)
        await client.query(`
            DELETE FROM v2.set_logs 
            WHERE block_exercise_id IN (
                SELECT id FROM v2.block_exercises WHERE block_id IN (
                    SELECT id FROM v2.workout_blocks WHERE workout_id = $1
                )
            )
        `, [workoutId]);
        ...
        console.log("✅ Performance Protocol + PLANK re-seeded successfully.");
